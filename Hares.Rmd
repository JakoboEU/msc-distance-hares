...
data$smeared_angle = abs(data$angle + (data$smear * 20))
data$new_distance = abs(round(data$true_distance * sin(data$smeared_angle * pi / 180)))
```
...
par(mfrow=c(2,2))
# histograms
hist(data$distance,main="",xlab="Perpendicular Distance (m)", breaks=12)
hist(data$true_distance,main="",xlab="Distance from Observer (m)", breaks=12)
...
data$new_distance_10 = abs(round(data$true_distance * sin(data$smeared_angle_10 * pi / 180)))
data$smeared_angle_20 = abs(data$angle + (data$smear * 20))
data$new_distance_20 = abs(round(data$true_distance * sin(data$smeared_angle_20 * pi / 180)))



hist(data$smeared_angle_10,main="Smeared with 10 degree range",xlab="Angle (degrees)", breaks=90)
hist(data$new_distance_10,main="",xlab="Perpendicular Distance (m)", breaks=12)

hist(data$smeared_angle_20,main="Smeared with 20 degree range",xlab="Angle (degrees)", breaks=90)
hist(data$new_distance_20,main="",xlab="Perpendicular Distance (m)", breaks=12)

```

Many animals at angle 0 are very close to the observer, so the affects of smearing the angle will be minimal
```{r}
hist(data$true_distance[data$angle < 10],main="",xlab="Distance (m) from observer", breaks=100)
```
...
clusters <- data[data[, "distance"] != NA, ]

print(data)
```

```{r}
d1 <- hist(data$new_distance,main="",xlab="Distance after smearing", plot = FALSE)
...
```{r}
model <- ds(data, key="hr", adjustment="herm", truncation=400,
