...
data$smear = runif(nrow(data), max=1, min=0)
...
```{r}
p1 <- hist(data$smeared_angle,main="",xlab="Smeared angle", plot = FALSE)
p2 <- hist(data$angle,main="",xlab="Angle", plot = FALSE)
...
```{r}
smeared.data <- rename(subset(data, select=c("object","size","new_distance")), c("new_distance"="distance")) 
```

```{r}
d.uniform.poly <- ds(data, key="hr", adjustment="herm", truncation = 400,
             region.table = regions, sample.table = samples, obs.table = observations)
s.uniform.poly <- ds(smeared.data, key="hr", adjustment="herm", truncation = 400,
             region.table = regions, sample.table = samples, obs.table = observations)
```

```{r}
ds.gof(d.uniform.poly)
ds.gof(s.uniform.poly)
```